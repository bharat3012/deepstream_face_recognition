...
SVM_CLASSIFIER_DICT = {"trained": False, "classifier": None}
...
def predict_using_classifier(faces_embeddings, labels, face_to_predict_embedding, threshold=85):
...
'''
def get_classifier(faces_embeddings=None, labels=None, classifier_name='svm'):
    if classifier_name == 'knn':
        ...
        if (SVM_CLASSIFIER_DICT["trained"] == False):
            classifier = train_classifier(faces_embeddings, labels, classifier_name=classifier_name)
            SVM_CLASSIFIER_DICT["classifier"] = classifier
            SVM_CLASSIFIER_DICT["trained"] = True
        else:
            #print('[INFO] Trained classifier assigned...')
            classifier = SVM_CLASSIFIER_DICT["classifier"]

    #print('[INFO] Classifier Training Done...')

    return classifier

def train_classifier(faces_embeddings, labels, classifier_name='svm'):
    '''
    ...
    if classifier_name == 'knn':
        classifier = KNeighborsClassifier(n_neighbors=5, p=2, weights="distance", metric="euclidean")
